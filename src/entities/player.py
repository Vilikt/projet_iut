...
class Player(AliveEntity, GameLoopInterface):
    ...
    def __handle_buttons_directions_and_b(self, delta: float, friction_value: float):
        ...
        if self.__is_direction_pressed:
            ...
            if (self._current_direction.x > 0 and self.__is_key_press(conf.button_left)) or \
                ...
                logger.debug(
                    f"On change de direction alors que la force de déplacement horizontale n'est pas nulle.")
    ...
    def handle_events(self, event: Event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == conf.button_a and (self.is_current_state(EntityStateName.DESCENDING) or self.is_current_state(EntityStateName.FALLING)):
                logger.debug(f"Le bouton A vient d'être appouyé alors que Mario est à l'état DESCENDING")
